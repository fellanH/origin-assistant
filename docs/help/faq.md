...
Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [Origin setup](/start/origin) and [Agent send](/tools/agent-send).
